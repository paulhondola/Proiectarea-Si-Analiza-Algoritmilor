...
TTree InsertNode(TTree a, TNodeRef parent, TKey key) {
  ...
  if (a.size >= MAXNODES - 1)
    return a;

  if (parent < 0 || parent > a.size)
    return a;
...
void printNode(TNode node, char *message) {
  printf("\n%s Key: %d, FS: %d, RS: %d\n", message, node.key, node.firstChild,
         node.rightSibling);
}
...
TNodeRef findNodeIndex(TTree a, TNode node) {
  for (uint8_t i = 0; i <= a.size; i++) {
    if (a.nodes[i].key == node.key &&
        a.nodes[i].firstChild == node.firstChild &&
        a.nodes[i].rightSibling == node.rightSibling)
      return i;
  }
  return 0;
}
...
TNode nodeByRef(TTree a, TNodeRef noderef) { return a.nodes[noderef]; }
...
void preOrderBinary(TTree a) { preOrderBinaryDo(a, getRoot(a)); }

void inOrderBinary(TTree a) { inOrderBinaryDo(a, getRoot(a)); }

void postOrderBinary(TTree a) { postOrderBinaryDo(a, getRoot(a)); }
